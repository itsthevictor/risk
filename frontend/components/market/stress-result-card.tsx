...
export function StressResultCard({
  ...
  const { stress } = useDictionary().market;
  return (
    <div className='h-full space-y-2 rounded-lg border p-4'>
      ...
      <div className='text-muted-foreground text-sm tabular-nums'>
        {stress.endingValue} {formatUsd(result.ending_value)}
      </div>
      {result.annualized_volatility != null && (
        <div className='text-muted-foreground text-sm tabular-nums'>
          {stress.annualizedVolatility}{' '}
